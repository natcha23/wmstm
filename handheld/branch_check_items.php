...
<div id="menu-name">สาขาตรวจนับ</div>
...
$db->select("*, obc.id as chk_id, rt.id as rt_id, rt.user_id as rt_user")->from("outbound_rt AS rt");
...
$db->join("outbound_branch_check AS obc", "rt.id = obc.outbound_id", "LEFT");
$db->where(array(
		"rt.status" => 0,
		"rt.rt_refid" => $rtID,
// 		"rt.rt_success" => 0
...
?>
    <tr <?php if($loop%2 == 0) {?> bgcolor="#FFFFFF" <?php } else { ?> bgcolor="#D8D8D8" <?php } ?>>
        ...
        <td><b><span id="ob-qty_<?php echo $row['barcode']; ?>"><?php echo $row['check_qty'];?></span></b></td>
        <td align="center"><input type="checkbox" name="data[<?php echo $loop; ?>][check_status]" <?php if ( !is_null($row['check_status']) && $row['check_status'] == 0) {?> checked <?php } ?> ></td>
...
?>
	...
	<button type="submit" onclick="">บันทึก</button><span style="margin-left:80px"><a href="" onclick="$.backShowCheck('<?php echo $today; ?>');" border='1'>ย้อนกลับ</a></span>
...
$(function(){
	$.backShowCheck = function(todate) {
		...
		$.postAndRedirect('?page=show_branch_check', params);
	}
...
// 				alert('บันทึกข้อมูลแล้ว');
	...
	$('#frm-brn-check').submit(function (e) {
		event.returnValue = false;
		if( $('input[type="checkbox"]:checked').length <=0 ) {
	        ...
	        event.returnValue = false;
	    	 ...
	    	 if (confirm('ต้องการบันทึกข้อมูลใช่หรือไม่') == true) {
				...
				$.ajax({
					type: 'post',
					url: '?page=branch_check_process',
					data: $('#frm-brn-check').serialize(),
					success: function (html) {
// 						alert('บันทึกข้อมูลแล้ว');
						console.log(html);
// 		 				location.reload();
...
//			event.returnValue = false;
	 if (confirm('ต้องการปิดรายการใช่หรือไม่') == true) {
		$.ajax({
			type: 'post',
			url: '../show/outbound/update_process.php',
			data: {	'method' : 'send-to-branch',
					'rt_id' : $('#rt_no').val()},
			success: function (html) {
//	 			alert('บันทึกข้อมูลแล้ว');
// 		 		location.reload();
