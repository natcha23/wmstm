...
while( $result = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
	...
	$rt = array(
			"user_id" => $user_id,
			"rt_refid" => $result['DI_REF'],
			"rt_date" => $result['DI_CRE_DATE']->format('Y-m-d H:i:s'),
			"rt_qty" => str_replace(',', '', $qty[0]),
			"rt_success" => 0,
			"barcode" => $result['TRD_SH_CODE'],
			"goods_name" => $result['GOODS_ALIAS'],
			"unit" => $result['TRD_UTQNAME'],
			"qty_amount" => 0,
			"remark" => null,
			"date_create" => $now,
			"date_update" => $now,
	);
	$CRE_DATE = $result['DI_CRE_DATE']->format('Y-m-d H:i:s');
	$db->select("*")->from("outbound_rt");
	$db->where(array("rt_refid" => $result['DI_REF'], "rt_date" => $CRE_DATE, "barcode" => $result['TRD_SH_CODE']));
	$sql = $db->get();
	
	
	
	$db->insert("outbound_rt", $rt);
